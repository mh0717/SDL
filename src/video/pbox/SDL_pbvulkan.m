...
#if SDL_VIDEO_VULKAN && SDL_VIDEO_DRIVER_PBOX
...
int PB_Vulkan_LoadLibrary(_THIS, const char *path)
{
...
void PB_Vulkan_UnloadLibrary(_THIS)
{
    if (_this->vulkan_config.loader_handle) {
...
SDL_bool PB_Vulkan_GetInstanceExtensions(_THIS,
                                          SDL_Window *window,
                                          unsigned *count,
                                          const char **names)
{
...
SDL_bool PB_Vulkan_CreateSurface(_THIS,
                                  SDL_Window *window,
                                  VkInstance instance,
                                  VkSurfaceKHR *surface)
{
    ...
    metalview = PB_Metal_CreateView(_this, window);
    if (metalview == NULL) {
        return SDL_FALSE;
    }
    ...
    if (vkCreateMetalSurfaceEXT) {
        ...
        createInfo.pLayer = (__bridge const CAMetalLayer *)
                            PB_Metal_GetLayer(_this, metalview);
        result = vkCreateMetalSurfaceEXT(instance, &createInfo, NULL, surface);
        if (result != VK_SUCCESS) {
            PB_Metal_DestroyView(_this, metalview);
...
void PB_Vulkan_GetDrawableSize(_THIS, SDL_Window *window, int *w, int *h)
{
    PB_Metal_GetDrawableSize(_this, window, w, h);
}
