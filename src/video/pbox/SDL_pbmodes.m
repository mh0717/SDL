...
@implementation SDL_PBDisplayData
...
@implementation SDL_PBDisplayModeData

@synthesize uiscreenmode;

@end

@interface SDL_PBDisplayWatch : NSObject
@end

@implementation SDL_PBDisplayWatch
...
+ (void)screenConnected:(NSNotification*)notification
{
    ...
    PB_AddDisplay(uiscreen, SDL_TRUE);
}

+ (void)screenDisconnected:(NSNotification*)notification
{
    UIScreen *uiscreen = [notification object];
    PB_DelDisplay(uiscreen);
}
...
{
    SDL_PBDisplayModeData *data = nil;

    if (uiscreenmode != nil) {
        /* Allocate the display mode data */
        data = [[SDL_PBDisplayModeData alloc] init];
        if (!data) {
...
int
PB_AddDisplay(UIScreen *uiscreen, SDL_bool send_event)
{
    ...
    if (PB_IsDisplayLandscape(uiscreen) != (size.width > size.height)) {
    ...
    SDL_PBDisplayData *data = [[SDL_PBDisplayData alloc] initWithScreen:uiscreen];
    if (!data) {
...
void
PB_DelDisplay(UIScreen *uiscreen)
{
    int i;

    for (i = 0; i < SDL_GetNumVideoDisplays(); ++i) {
        SDL_PBDisplayData *data = (__bridge SDL_PBDisplayData *)SDL_GetDisplayDriverData(i);
...
void SDL_PBOnApplicationDidChangeStatusBarOrientation(void);
int
PB_InitModes(_THIS)
{
    @autoreleasepool {
        for (UIScreen *uiscreen in [UIScreen screens]) {
            if (PB_AddDisplay(uiscreen, SDL_FALSE) < 0) {
                return -1;
            }
        }
#if !TARGET_OS_TV
        SDL_PBOnApplicationDidChangeStatusBarOrientation();
#endif

        [SDL_PBDisplayWatch start];
    }
...
void
PB_GetDisplayModes(_THIS, SDL_VideoDisplay * display)
{
    @autoreleasepool {
        SDL_PBDisplayData *data = (__bridge SDL_PBDisplayData *) display->driverdata;

        SDL_bool isLandscape = PB_IsDisplayLandscape(data.uiscreen);
...
int
PB_SetDisplayMode(_THIS, SDL_VideoDisplay * display, SDL_DisplayMode * mode)
{
    @autoreleasepool {
        SDL_PBDisplayData *data = (__bridge SDL_PBDisplayData *) display->driverdata;

#if !TARGET_OS_TV
        SDL_PBDisplayModeData *modedata = (__bridge SDL_PBDisplayModeData *)mode->driverdata;
        
        
        UIScreenMode* smode = modedata.uiscreenmode;
        UIScreen* sscreen = data.uiscreen;
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            [sscreen setCurrentMode:smode];
        }];
//    [data.uiscreen setCurrentMode:modedata.uiscreenmode];
        ...
        if (data.uiscreen == [UIScreen mainScreen]) {
             ...
             * the display mode. */
            if (mode->w > mode->h) {
                if (!PB_IsDisplayLandscape(data.uiscreen)) {
...
#if !TARGET_OS_TV
void SDL_PBOnApplicationDidChangeStatusBarOrientation()
{
    __block UIInterfaceOrientation orient = UIInterfaceOrientationPortrait;
    if (NSThread.isMainThread) {
        orient = [UIApplication sharedApplication].statusBarOrientation;
